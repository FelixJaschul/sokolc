...
SOKOL_GFX_API_DECL void *sg_create_state(void);
SOKOL_GFX_API_DECL void sg_set_state(void *state);
SOKOL_GFX_API_DECL void sg_destroy_state(void *state);
...
#pragma warning(disable:4055)   /* 'type cast': from data pointer */
    ...
    #endif
    #define _SG_GL_CHECK_ERROR() do {\
        int err = glGetError();\
        if (err) { fprintf(stderr, "gl error %d", err); }\
        SOKOL_ASSERT(err == GL_NO_ERROR);\
    } while (0);
#endif
...
_SOKOL_PRIVATE GLenum _sg_gl_min_filter(sg_filter f) {
    switch (f) {
...
_SOKOL_PRIVATE GLenum _sg_gl_mag_filter(sg_filter f) {
    switch (f) {
        case SG_FILTER_NEAREST:                 return GL_NEAREST;
        case SG_FILTER_LINEAR:                  return GL_LINEAR;
        case SG_FILTER_NEAREST_MIPMAP_NEAREST:  return GL_NEAREST;
        case SG_FILTER_NEAREST_MIPMAP_LINEAR:   return GL_NEAREST;
        case SG_FILTER_LINEAR_MIPMAP_NEAREST:   return GL_LINEAR;
        case SG_FILTER_LINEAR_MIPMAP_LINEAR:    return GL_LINEAR;
        default: SOKOL_UNREACHABLE; return 0;
    }
}
...
_SOKOL_PRIVATE sg_resource_state _sg_gl_create_image(_sg_image_t* img, const sg_image_desc* desc) {
    ...
    _SG_GL_CHECK_ERROR();
            ...
            const GLenum gl_min_filter = _sg_gl_min_filter(img->cmn.min_filter);
            const GLenum gl_mag_filter = _sg_gl_mag_filter(img->cmn.mag_filter);
        ...
        */
        for (GLuint i = 0; i < (GLuint)pip->cmn.color_count; i++) {
                ...
                (state_bs->dst_factor_alpha != cache_bs->dst_factor_alpha))
            {
                ...
                glBlendFuncSeparatei(i,_sg_gl_blend_factor(state_bs->src_factor_rgb),
            ...
            if ((state_bs->op_rgb != cache_bs->op_rgb) || (state_bs->op_alpha != cache_bs->op_alpha)) {
                ...
                glBlendEquationSeparatei(i,
                    _sg_gl_blend_op(state_bs->op_rgb), _sg_gl_blend_op(state_bs->op_alpha));
            }
        }
...
_SOKOL_PRIVATE bool _sg_validate_apply_pipeline(sg_pipeline pip_id) {
        ...
        if (pass) {
            ...
            if (pip->cmn.color_count != pass->cmn.num_color_atts) {
                fprintf(stderr, "%d, %d\n", pip->cmn.color_count, pass->cmn.num_color_atts);
            }
...
//
// >>public
SOKOL_API_IMPL void *sg_create_state(void) {
    return calloc(1, sizeof(_sg_state_t));
}

SOKOL_API_IMPL void sg_set_state(void *state) {
    _sg = state;
}

SOKOL_API_IMPL void sg_destroy_state(void *state) {
    free(state);
}
