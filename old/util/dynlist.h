...
#define DYNLIST_LOCKED (1 << 0)

// internal use only
// header of dynlist
typedef struct {
    int size, capacity, flags;
    void *userdata;
...
STATIC_ASSERT(
    sizeof(dynlist_header) % 16 == 0,
    "dynlist_header has bad size");
...
// internal use only
// frees a dynlist
void _dynlist_free_impl(void **plist);
...
void _dynlist_remove_impl(void **plist, int tsize, int index, int flags);
...
#define dynlist_header(_q) ({                                                \
        TYPEOF(_q) __q = (_q);                                               \
        (dynlist_header*) (                                                  \
            __q ? (((void*) __q) - sizeof(dynlist_header)) : NULL);          \
    })

// TODO
#define dynlist_lock(_q) (dynlist_header((_q))->flags |= DYNLIST_LOCKED)

// TODO
#define dynlist_unlock(_q) (dynlist_header((_q))->flags &= ~DYNLIST_LOCKED)

// returns POINTER TO header userdata pointer, NULL if it does not exist or
// list is NULL
#define dynlist_userdata_ptr(_d) ({                                          \
        dynlist_header *_h = dynlist_header((_d));                           \
        _h ? &_h->userdata : NULL;                                           \
    })
...
#define dynlist_append(_d) ({                                                \
        TYPEOF(_d) *_pd = &(_d);                                             \
        dynlist_header *h = dynlist_header(*_pd);                            \
        (TYPEOF(&(_d)[0])) _dynlist_insert_impl(                             \
...
#define dynlist_remove(_d, _i) ({                                            \
        ...
        _dynlist_remove_impl( \
            (void**) _pd, sizeof(*(_d)) /* NOLINT */, __i, _DYNLIST_RF_NONE); \
        _t;                                                                  \
    })
...
void _dynlist_alloc_impl(void **plist, int tsize) {
    dynlist_header *h =
        malloc(
            sizeof(dynlist_header) + (tsize * DYNLIST_INIT_CAP));
    *h = (dynlist_header) {
        .size = 0,
...
void _dynlist_free_impl(void **plist) {
    if (*plist) {
        free(*plist - sizeof(dynlist_header));
        *plist = NULL;
    }
}
...
void _dynlist_realloc_impl(void **plist, int tsize, int newcap, int flags) {
    ...
    dynlist_header *h = dynlist_header(*plist);

    if ((h->flags & DYNLIST_LOCKED)) {
        ASSERT(
            newcap <= h->capacity,
            "can't realloc locked dynlist %p (%d -> %d)",
            *plist,
            h->capacity,
            newcap);
        return;
    ...
    if (h->capacity != capacity) {
        h->capacity = capacity;
        h = realloc(h, sizeof(*h) + (h->capacity * tsize));
        *plist = h + 1;
    }
}
...
void _dynlist_copy_from_impl(void **pdst, void **psrc, int tsize) {
    if (*psrc) {
        dynlist_header *h = dynlist_header(*psrc);
