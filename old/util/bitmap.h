...
ALWAYS_INLINE BITMAP *bitmap_alloc(int sz) {
    return malloc(BITMAP_SIZE_TO_BYTES(sz));
}

ALWAYS_INLINE BITMAP *bitmap_calloc(int sz) {
  BITMAP *res = malloc(BITMAP_SIZE_TO_BYTES(sz));
  memset(res, 0, BITMAP_SIZE_TO_BYTES(sz));
  return res;
}

ALWAYS_INLINE BITMAP *bitmap_realloc(BITMAP *b, int sz) {
    return realloc(b, BITMAP_SIZE_TO_BYTES(sz));
}

ALWAYS_INLINE BITMAP *bitmap_realloc_zero(BITMAP *b, int old, int sz) {
    const int
        b_old = BITMAP_SIZE_TO_BYTES(old),
        b_new = BITMAP_SIZE_TO_BYTES(sz);

    BITMAP *p = realloc(b, b_new);

    if (b_old < b_new) {
        memset(&p[b_old], 0, b_new - b_old);
    }
...
ALWAYS_INLINE int bitmap_find(
    ...
    else if (start >= size) { return INT_MAX; }
