...
/* static void _sg_gl_query_image_pixels(_sg_image_t* img, int x, int y, int width, int height, void* pixels) { */
/*     SOKOL_ASSERT(img->gl.target == GL_TEXTURE_2D); */
/*     SOKOL_ASSERT(0 != img->gl.tex[img->cmn.active_slot]); */
/* /1* #if defined(SOKOL_GLCORE33) *1/ */
/* /1*     _sg_gl_cache_store_texture_binding(0); *1/ */
/* /1*     _sg_gl_cache_bind_texture(0, img->gl.target, img->gl.tex[img->cmn.active_slot]); *1/ */
/* /1*     glGetTextureSubImage(); *1/ */
/* /1*     glGetTexImage(img->gl.target, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels); *1/ */
/* /1*     _SG_GL_CHECK_ERROR(); *1/ */
/* /1*     _sg_gl_cache_restore_texture_binding(0); *1/ */
/* /1* #else *1/ */
...
static void _sg_gl_query_image_pixels(_sg_image_t* img, void* pixels) {
    ...
    GLenum format = _sg_gl_teximage_format(img->cmn.pixel_format);
    GLenum type = _sg_gl_teximage_type(img->cmn.pixel_format);
    glGetTexImage(img->gl.target, 0, format, type, pixels);
    _SG_GL_CHECK_ERROR();
...
void sg_query_image_pixels(sg_image img_id, void* pixels, int size) {
    ...
    SOKOL_ASSERT(img);
    /* TODO: check actual pixel size SOKOL_ASSERT(size >= (img->cmn.width * img->cmn.height * 4)); */
    _SOKOL_UNUSED(size);
#if defined(_SOKOL_ANY_GL)
